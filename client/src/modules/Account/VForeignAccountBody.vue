<template>
  <q-card class="q-mb-md" flat>
    ...
    <q-card-section>
      <div class="text-body1">Обо мне: {{ userProfile?.bio }}</div>
      <div class="text-body1">Работа: {{ userProfile?.company }}</div>
      <div class="text-body1">Местоположение: {{ userProfile?.location }}</div>
    </q-card-section>
    ...
    <q-card-section>
      <q-chip
        ...
        text-color="white"
        icon="link"
        >{{ social.resource }}{{ social.link }}</q-chip
      >
    </q-card-section>
  </q-card>
